...
(async function(){
    try {
        ...
        let name = await cTab.evaluate(function(select){return document.querySelector(select).innerText} ,'.yt-simple-endpoint.style-scope.yt-formatted-string' )
...
function getLength(durattionSelect){
      let duratioElem = document.querySelectorAll(durattionSelect)
